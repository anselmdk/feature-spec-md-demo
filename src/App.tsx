...
import { createTicket, filterTickets, transitionTicket, type Ticket, type TicketFilter } from "./ticketRules";

const initialTickets: Ticket[] = [
  { id: "seed-open", title: "Printer is offline", status: "open" },
  { id: "seed-progress", title: "Email delivery is slow", status: "in-progress" },
  { id: "seed-resolved", title: "Password reset loop", status: "resolved", resolutionNote: "Reset link cache was cleared." },
];
...
export function App() {
  ...
  const visibleTickets = useMemo(() => filterTickets(tickets, filter), [tickets, filter]);
  ...
  function handleCreateTicket(event: FormEvent) {
    ...
    try {
      ...
      setTitleError(error instanceof Error ? error.message : "Ticket could not be created.");
    }
  }
    ...
    <main className="app">
      ...
      <section className="panel" aria-labelledby="create-ticket-heading">
        ...
        <form onSubmit={handleCreateTicket}>
          ...
          <input id="ticket-title" value={title} onChange={(event) => setTitle(event.target.value)} placeholder="What needs help?" />
      ...
      <section className="panel" aria-labelledby="filters-heading">
        ...
        <div className="filters">
          {(["all", "open", "in-progress", "resolved"] as TicketFilter[]).map((value) => (
        ...
        <ul className="tickets">
          {visibleTickets.map((ticket) => (
            <li key={ticket.id} className="ticket" data-testid="ticket-card">
              ...
              {ticket.status === "open" ? (
                <button type="button" onClick={() => startTicket(ticket)}>Start work</button>
              ) : null}

              {ticket.status === "in-progress" ? (
                <div className="resolve">
                  <label htmlFor={`resolution-${ticket.id}`}>Resolution note</label>
                  <input id={`resolution-${ticket.id}`} value={resolutionNotes[ticket.id] ?? ""} onChange={(event) => setResolutionNotes((current) => ({ ...current, [ticket.id]: event.target.value }))} />
                  <button type="button" onClick={() => resolveTicket(ticket)}>Resolve</button>
                  {statusErrors[ticket.id] ? <p role="alert">{statusErrors[ticket.id]}</p> : null}
                </div>
              ) : null}

              {ticket.resolutionNote ? <p>Resolution: {ticket.resolutionNote}</p> : null}
            </li>
          ))}
        </ul>
      </section>
